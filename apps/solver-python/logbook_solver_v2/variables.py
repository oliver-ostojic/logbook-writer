...
class VariableBuilder:
    ...
    def build(
        ...
        *,
        ...
        # Identify pure HOURLY roles: HOURLY model only, not WINDOW-like, not DAILY,
        # not SOLVER/HOURLY_AND_SOLVER. These only get variables when coverage windows
        # exist for them — same principle as the DAILY and WINDOW guards.
        UNRESTRICTED_MODELS = {'SOLVER', 'HOURLY_AND_SOLVER'}
        pure_hourly_role_ids: set[int] = set()
        for role in role_records:
            all_models = set(role.get('assignmentModels') or [])
            single = role.get('assignmentModel') or ''
            if single:
                all_models.add(single)
            if (
                'HOURLY' in all_models
                and not (UNRESTRICTED_MODELS & all_models)
                and not (WINDOW_LIKE_MODELS & all_models)
                and role['id'] not in daily_role_ids
            ):
                pure_hourly_role_ids.add(role['id'])

        # Role IDs that appear in at least one coverage window in the payload.
        # Used to guard pure HOURLY roles — no coverage window means no variables.
        coverage_role_ids: set[int] = {
            cw['roleId'] for cw in (coverage_windows or []) if cw.get('roleId') is not None
        }
        ...
        for crew in crew_records_sorted:
            ...
            for role_id in sorted(eligible_role_ids):
                ...
                # For pure HOURLY roles, skip if no coverage windows exist for this role.
                if role_id in pure_hourly_role_ids and role_id not in coverage_role_ids:
                    if DEBUG: print(f"    Skipping HOURLY role {role_code_by_id.get(role_id)} for {crew_name}: no coverage windows", file=sys.stderr)
                    continue
