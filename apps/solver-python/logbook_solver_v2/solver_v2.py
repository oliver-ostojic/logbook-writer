...
class AssignmentIndex:
    ...
    def __init__(self, variables: Dict[AssignmentKey, cp_model.IntVar]):
        self.by_crew_slot: Dict[Tuple[str, int], List[Tuple[int, cp_model.IntVar]]] = defaultdict(list)
        self.by_slot: Dict[int, List[Tuple[int, cp_model.IntVar]]] = defaultdict(list)

        for (crew_id, slot, role_id), var in variables.items():
            self.by_crew_slot[(crew_id, slot)].append((role_id, var))
            self.by_slot[slot].append((role_id, var))

    def get(self, key: Tuple[str, int]) -> List[Tuple[int, cp_model.IntVar]]:
        return self.by_crew_slot.get(key, [])

    def get_by_slot(self, slot: int) -> List[Tuple[int, cp_model.IntVar]]:
...
class SolverV2:
    ...
    def __init__(self, payload: Dict[str, Any]):
        ...
        self.time_grid = TimeGrid.from_store(
            base_slot_minutes=self.store.get('baseSlotMinutes'),
    ...
    def solve(self, time_limit_seconds: int | None = None) -> Dict[str, Any]:
        ...
        if success:
            for (crew_id, slot, role_id), var in self.assignment_vars.items():
                if solver.Value(var):
                    start, end = self.time_grid.slot_to_minutes(slot)
                    assignments.append(
                        {
                            'crewId': crew_id,
                            'roleId': role_id,
